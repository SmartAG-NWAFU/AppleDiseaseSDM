...
def add_north(ax, labelsize=12, loc_x=0.95, loc_y=0.95, width=0.04, height=0.13, pad=0.14):
    """
...
def add_scalebar(ax, y=34.5, x=73.5, length_km=500, lw=3, size=10, lat_range=(10, 60), proj=ccrs.PlateCarree()):
    """
    ...
    ax.hlines(y=y, xmin=x, xmax=x+length_degree, 
              colors='black', lw=lw, transform=proj)
    
    # Ticks (at both ends and the middle)
    for pos in [0, 0.5, 1]:
        ax.vlines(x=x + pos*length_degree, ymin=y-0.15, ymax=y+0.15, 
                  colors='black', lw=lw-1, transform=proj)
    
    # Text annotations
    ax.text(x, y + 0.2, '0', ha='center', va='bottom', 
            fontsize=size, fontname='Times New Roman', transform=proj)
    ax.text(x + length_degree/2, y + 0.2, f'{int(length_km/2)}',
            ha='center', va='bottom', fontsize=size, fontname='Times New Roman', transform=proj)
    ax.text(x + length_degree, y + 0.2, f'{length_km} km', ha='center', 
            va='bottom', fontsize=size, fontname='Times New Roman', transform=proj)
